...
fn main() {
    ...
    let problem = problem::get_problem(id).expect(&format!("problem #{} not found", id));
    let code = problem
        .code_definition
        .iter()
    ...
    let source = template
        ...
        .replace("__PROBLEM_TEST_CASE__", &problem.sample_test_case)
    ...
    writeln!(lib_file, "mod {};", file_name).expect("failed to write problem file");
}
